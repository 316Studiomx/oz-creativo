...
test('checkout form validates coupon estimates without trusting frontend totals', () => {
  ...
  assert.equal(checkoutFormSource.includes('sm:grid-cols-[96px_minmax(0,1fr)]'), true)
  assert.equal(checkoutFormSource.includes('max-w-[96px]'), true)
  assert.equal(checkoutFormSource.includes('coupon-control-row'), true)
